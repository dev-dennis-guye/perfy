...
describe('Test: perfy', function () {
  ...
  it('should return elapsed time', function (done) {
    const timeout = 1233;
    const offset = 500;
    const timeoutMax = timeout + offset;
    const secs = timeout / 1000;
    const secsMax = secs + 1;
    const ms = timeout % 1000;
    const msMax = ms + offset;
    const nsMin = secs * 1000 * 1000000;

    perfy.start('m1');
    setTimeout(function () {
      const result = perfy.end('m1');
  ...
  function throw_exists() {
    return perfy.exists();
  }
  function throw_destroy() {
    return perfy.destroy();
  }
  function throw_result() {
    return this.result();
  }
  ...
  it('should save/exec named sync', function () {
    var name = 'sync-op',
      result = this.exec(name, function () {
        var i = 0;
        while (i < 10000) {
          i += 1;
        }
    ...
    expect(result).toBeDefined();
    expect(result.time).toEqual(jasmine.any(Number));
    expect(this.result(name)).toBeDefined();
    expect(this.exists(name)).toEqual(true);
  });

  it('should exec async', function (jasmineDone) {
    destroyAll().exec(function (done) {
      setTimeout(function () {
        var result = done();
        expect(result).toBeDefined();
